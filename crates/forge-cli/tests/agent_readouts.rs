...
//! fallback run and then read it back through `forge inspect`, the TUI's
//! model, the console's `/api/view` payload and `forge compare` — and
...
/// AC-18: `forge compare` reports a model difference as a FIRST-CLASS
...
fn no_surface_composes_the_provenance_sentence_itself() {
    ...
    let derivation = crates_dir.join("forge-view/src/lib.rs");
    ...
    for surface in [
        ...
        "forge-cli/src/tui.rs",
        "forge-cli/src/ui.html",
        "forge-cli/src/compare.rs",
    ] {
        ...
        for fragment in ["via {provider}", "fallback #", "first choice"] {
            assert!(
                ...
                "{surface} composes '{fragment}' itself; the sentence belongs \
                 to the single forge-view derivation"
            );
        }
    }
}
