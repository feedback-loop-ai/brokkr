//! Terminal rendering for `forge-view`'s models: pure string functions,
...
pub fn runs(view: &RunsView, now: &str, style: &Style) -> String {
    ...
    for run in &view.runs {
        ...
        let age = match forge_view::age(&run.created_at, now) {
            Some(age) => age,
...
fn trail_block(view: &RunView, lens: Option<&Lens>, style: &Style) -> String {
    ...
    for row in &rows {
        let line = format!(
            ...
            forge_view::clamp(Safe::new(&row.what.text).as_str(), remaining),
        );
        push_line(&mut out, &line);
    }
    out
