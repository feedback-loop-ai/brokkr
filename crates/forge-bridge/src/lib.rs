...
//!
//! The bridge reads only through `forge-store`'s verified public API. It never
...
use forge_core::dispatch::{bundle_manifest_from_run, dispatch_from_run, DispatchEnvelopeV2};
use forge_core::{EventEnvelope, EventType};
use forge_runtime::{apply_fenced_operator_command, FencedCommandOutcome};
...
pub enum BridgeError {
    ...
    #[error("runtime: {0}")]
    Runtime(#[from] forge_runtime::EngineError),
