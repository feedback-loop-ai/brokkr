//! `forge-view` — one derivation, two surfaces (decision 0013).
//!
...
//! on exactly `forge-core`, `serde` and `serde_json`, so that property is
...
use forge_core::fold::{RunState, Status};
...
/// existing `forge inspect` prints them.
#[derive(Serialize)]
pub struct Summary {
...
/// feature panics, and a panic in `forge runs` is worse than any
/// misalignment.
