...
/// F2 (medium): `collect_unpinned` flattens every AUTHORING coordinate
/// into one global label, and a phase name is an arbitrary string that
/// may itself contain `:`, so two structurally different owners can
/// write the same label. The second assessment — and its declaration
/// witness — silently replaces the first, then the engine delivers the
/// overwritten value to the selected Codex site; a Claude replacement
/// has no work-site shape, so an otherwise supported retry goes cold.
/// Refuse the ambiguity at its authoring coordinate, before any evidence
/// is collected, naming the shared label and both owners.
#[test]
fn two_owners_of_one_global_authoring_label_are_refused_before_collection() {
    let root = workspace_root();
    let codex = || {
    ...
    let compile = |config: &Value, policy: &Value| {
        let fixture = Fixture::new();
        std::fs::write(fixture.dir.path().join("bundle.json"), config.to_string()).unwrap();
        std::fs::write(fixture.dir.path().join("policy.json"), policy.to_string()).unwrap();
        Bundle::compile_with(
    ...
    let policy = |outer: &str| {
        json!({
            "phases":["work", outer, "review", "done"], "initial":"work",
            "terminal":["done"],
            "rules":[
                {"id":"W","from":"work","result":"complete","next":"review","reason":"work"},
                {"id":"O","from":outer,"result":"complete","next":"review","reason":"outer"},
                {"id":"R","from":"review","result":"clean","next":"done","reason":"review"},
            ],
        })
    };

    // (a) cross-phase: phase `work`, case `chore`, beside the literal
    // phase `work:chore`. Both authoring coordinates flatten to
    // `work:chore`.
    let config = |outer: &str| {
        json!({
            "name":"alias", "policy":"policy.json", "protected_phase":"review",
            "seats":{
                "work":{"results":["complete"],"select":{
                    "on":"strategy","cases":{"chore":codex()},"default":claude()}},
                outer:{"results":["complete"],"role":"roles/role.md",
    ...
    let refusal = error(compile(&config("work:chore"), &policy("work:chore")));
    assert!(refusal.contains("both address 'work:chore'"), "{refusal}");
    assert!(refusal.contains("'work' > 'chore'"), "{refusal}");
    assert!(refusal.contains("'work:chore'"), "{refusal}");
    // The renamed control compiles and the Codex site keeps its own
    // supported assessment: the guard refuses the collision, not colons.
    let control =
        compile(&config("work-chore"), &policy("work-chore")).expect("renamed control compiles");
    assert_eq!(
        control.inline_resume["work:chore"]["work-site"]["status"], "supported",
        "the renamed control's Codex site keeps its own assessment"
    );

    // (b) cross-selector: `work`/`chore`/step `engine:stage` beside
    // `work:chore`/`engine`/step `stage`; both flatten to
    // `work:chore:engine:stage`.
    let sequenced = |first: &str| {
        json!([
            {"name":first,"results":["pass"],"role":"roles/role.md",
             "driver":{"command":["{brokkr}","driver","codex","--","--model",
                "gpt-6-astra","--effort","xhigh","--sandbox","danger-full-access"]}},
            {"name":"tail","role":"roles/role.md",
             "driver":{"command":["{brokkr}","driver","claude","--","--model",
                "claude-sonnet-4","--effort","high"]}},
        ])
    };
    let select_config = |outer: &str| {
        json!({
            "name":"alias", "policy":"policy.json", "protected_phase":"review",
            "seats":{
                "work":{"results":["complete"],"select":{
                    "on":"strategy","cases":{"chore":json!({
                        "sequence":sequenced("engine:stage")})},
                    "default":claude()}},
                outer:{"results":["complete"],"select":{
                    "on":"strategy","cases":{"engine":json!({
                        "sequence":sequenced("stage")})},
                    "default":claude()}},
                "review":{"results":["clean"],"role":"roles/role.md",
                    "driver":{"command":["driver"]}},
            },
    ...
    let refusal = error(compile(&select_config("work:chore"), &policy("work:chore")));
    assert!(
        refusal.contains("both address 'work:chore:engine:stage'"),
        "{refusal}"
    );
    assert!(refusal.contains("'engine:stage'"), "{refusal}");
    assert!(
        refusal.contains("'work:chore' > 'engine' > 'stage'"),
        "{refusal}"
    );
    let control = compile(&select_config("work-chore"), &policy("work-chore"))
        .expect("renamed cross-selector control compiles");
    assert_eq!(
        control.inline_resume["work:chore:engine:stage"]["work-site"]["status"], "supported",
        "the renamed control's Codex step keeps its own assessment"
    );

    // A site that legitimately shares a PREFIX with a wrapper destination
    // is not a collision: phase `verify:foo` is its own owner, and an
    // ordinary repeated member name under two steps still compiles.
    let plain = json!({
        "name":"plain", "policy":"policy.json", "protected_phase":"review",
        "seats":{
    ...
    assert!(compile(&plain, &plain_policy).is_ok());
}

/// F2 (medium), the wrapper's own coordinate: the dialect wrapper MOVES
/// the verify seat's evidence to `verify:checks` / `verify:checks:<member>`.
/// A literal phase that already owns that destination must not be
/// silently replaced by the relocation. The refusal names the occupied
/// destination rather than letting the two sites aliases.
#[test]
fn a_wrapper_destination_already_owned_by_another_site_is_refused() {
    let root = workspace_root();
    let fixture = Fixture::new();
    let dialect = Dialect::load(&root.join("dialects/openspec.json"))
        .unwrap()
        .0;
    let policy = json!({
        "phases":["design","verify","verify:checks","review","done"],
        "initial":"design", "terminal":["done"],
        "rules":[
            {"id":"D","from":"design","result":"drafted","next":"verify","reason":"drafted"},
            {"id":"DF","from":"design","result":"fail","next":"design","reason":"retry"},
            {"id":"V","from":"verify","result":"pass","next":"review","reason":"pass"},
            {"id":"VF","from":"verify","result":"fail","next":"verify","reason":"retry"},
            {"id":"VC","from":"verify:checks","result":"pass","next":"review","reason":"checks"},
            {"id":"VCF","from":"verify:checks","result":"fail","next":"verify","reason":"retry"},
            {"id":"R","from":"review","result":"clean","next":"done","reason":"clean"}
        ]
    });
    let config = json!({
        "name":"destination-alias", "policy":"policy.json", "protected_phase":"review",
        "seats":{
            "design":{"results":["drafted","fail"],"sequence":[
                {"name":"author","results":["drafted"],"role":"roles/role.md",
                 "driver":{"command":["driver"]}},
                {"name":"validate","dialect":"validate"}]},
            "verify":{"results":["pass","fail"],"class":"gate","hands":"workspace",
                "role":"roles/role.md",
                "driver":{"command":["{brokkr}","driver","codex","--","--model",
                    "gpt-6-astra","--effort","xhigh","--sandbox","danger-full-access"]}},
            "verify:checks":{"results":["pass","fail"],"role":"roles/role.md",
                "driver":{"command":["{brokkr}","driver","claude","--","--model",
                    "claude-sonnet-4","--effort","high"]}},
            "review":{"results":["clean"],"role":"roles/role.md",
                "driver":{"command":["driver"]}},
    ...
    assert!(refusal.contains("verify:checks"), "{refusal}");
    assert!(refusal.contains("another site already owns"), "{refusal}");
}
