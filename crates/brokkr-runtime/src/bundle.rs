...
pub struct Bundle {
    ...
    pub manifest: Value,
    /// Decision 0040: the sites whose hands are one boxed tool, keyed as
...
struct ResolvedSeat {
    ...
    /// Decision 0040: the agent's hands, when it declared them.
...
fn enforce_model_policy(
    ...
    for (driver, pin) in drivers {
        ...
        // Decision 0040 ruling 3: a deterministic `exec` command whose
...
/// Decision 0040: record one site's hands — the agent's when the site
...
fn record_hands(
    ...
    let spec = match (from_agent, raw.get("hands")) {
        ...
        (None, Some(declared)) => HandsSpec::parse(declared).map_err(|problem| {
            CompileError::Invalid(format!("seat '{what}' hands: {problem} (decision 0040)"))
        })?,
        (None, None) => return Ok(()),
    };
    if !secrets.is_empty() {
...
fn manifest_for(
    ...
    // ABSENT on the same terms once more (decision 0040): a bundle that
