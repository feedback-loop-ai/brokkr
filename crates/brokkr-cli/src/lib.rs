...
fn run_with(
    ...
    match cli.command {
        Cmd::Init { dir } => {
            ...
            // The scaffold carries its own `adapters/` and `agents/`, where
            // the trust tier its gate seats compile against and the tool
            // allowances its seats run under are declared (decisions 0021
            // and 0016). Every other verb reads those trees from the
