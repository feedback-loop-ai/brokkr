...
/// says so ONCE — the scaffold has five seats resolving through the one
/// adapter, each with a two-model chain, so a per-expansion notice would
/// show up here as ten lines — on stderr, never on stdout. The token
/// lives in the adapter's driver now: the seats name agents, and the
/// driver prefix is the adapter's to declare.
#[test]
