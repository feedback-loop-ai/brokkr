...
use forge_runtime::{resolve_agent, Adapters, Availability, Library};
...
fn every_shipped_agent_resolves_at_compile_time() {
    ...
    for name in AGENTS {
        ...
        // The composed argv keeps `{forge}` a literal: expansion is the
        // compiler's job, and a machine-local path never reaches a digest.
        assert_eq!(resolution.candidates[0].argv[0], "{forge}");
    }
}
