...
/// the agent's declared tool order, with `{forge}` still a literal.
#[test]
...
/// than thrown, so `forge agents show` can print the whole chain.
#[test]
...
/// `{forge}` expansion is a machine-local absolute path.
#[test]
...
/// The library exposes what `forge agents list` prints, in name order.
#[test]
