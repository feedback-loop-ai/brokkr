...
Name which DSH session format versions the reader admits and why, keep
version 3 refused until its writer has been read, and pin the facts about
it that are measurable without that writer.
...
is closed and is exactly `{0}`. Numeric zero spellings such as `0`, `0.0`,
`0e0` and `-0` SHALL denote version zero; strings, booleans and null SHALL
not be coerced. Missing `version`, any nonnumeric value and any numeric
value outside the set SHALL return `unsupported-format` with
...
Version zero is admitted because its writer was read: the 0.1.2-rc.1
session and persistence-JSONL packages captured for #222 declare
`SESSION_FORMAT_VERSION = 0`, write that header, refuse a foreign version
before decoding, and supply the event catalogue, packed-row and citation
types behind the version-zero rules below. Version 3, which the 0.1.5-rc.1
core writes into `session.v3.jsonl`, is deliberately not admitted: that
core's writer has not been read through any hands of this repository, and
a versioned filename, a header number or the resemblance of sampled rows to
admitted shapes is not admission evidence. A version SHALL join the set
only through a change that records, from the writer's own source, its
package identity and captured files with digests; the record vocabulary it
emits; how streaming fragments persist; the meaning of every header field
it adds; the meaning of `surfaceOp` and `sourceEventSeqs` on every row that
carries them; a disposition for every record type outside the admitted
vocabulary; and whether every admitted-version meaning is preserved or a
per-version projection is required. Until then a version outside the set
keeps the refusal below, whatever its rows resemble. The filename and the
header version are independent facts: `session.v3.jsonl` carrying a
version-zero header is readable under the version-zero rules, and
`session.jsonl` carrying version 3 is refused; neither infers the other.
...
mistyped. In particular `isSeeded` SHALL neither establish nor remove
ownership, SHALL not alter the delegation-depth rule and SHALL not supply
admission; what a seeded session means for provenance is unmeasured and no
reader claim rests on it. Extra fields SHALL be ignored, never interpreted
as an event, identity, path, timestamp or execution policy. This is the
complete reader header-admission shape; it is deliberately not DSH's
execution-replay header validator.
...
#### Scenario: A version-3 session is located and refused until its writer is read
- **WHEN** the unique safe DSH candidate is `session.v3.jsonl` whose opening row is `{"type":"session","version":3,"delegationDepth":0,"isSeeded":false}`, followed by complete rows typed `system/message`, `todo/write`, `turn/end`, `user/message` and `assistant/message` within both caps, with discovery complete and no I/O or UTF-8 failure
- **THEN** discovery resolves that path and content admission returns `unsupported-format` with `DSH transcript format is not supported`, the unchanged reference, `legacy: false`, the confirmed path and DSH hint, no turns, zero counts, `truncated: false` and no notices; no later row is classified, so those sampled types gain no disposition by being present
- **AND** CLI text, CLI JSON and the TUI show that same refusal, and a `--turn` request cannot reach any of those rows

#### Scenario: The filename and the header version are independent facts
- **WHEN** one recorded root's only session directory holds `session.jsonl` whose opening header has version 3 and depth zero, and another recorded root's only session directory holds `session.v3.jsonl` whose opening header has version 0 and depth zero followed by a readable `user/message`
- **THEN** reading the first returns `unsupported-format` with its confirmed path and zero counts, and reading the second projects the message with zero counts and no notice; neither name infers a version and neither version infers a name

#### Scenario: A seeded header changes neither ownership nor admission
- **WHEN** the unique safe candidate's opening header has version 0, depth zero or omitted depth, and `isSeeded` true, false, a string or an object, followed by a readable `user/message`
- **THEN** the source is admitted and projects that message with zero counts and no notice, and the same header with version 3 instead returns `unsupported-format` with its confirmed path
...
- **THEN** the first read returns `unsupported-format` with no turns and one unrecognized record, because `system/message` is outside the captured 0.1.2-rc.1 catalogue and is a required unknown; the second projects the user message once with one unrecognized record and its notice
