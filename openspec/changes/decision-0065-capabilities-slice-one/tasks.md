...
The checked 2.1.7 below retains the prior visit's observed local gates, not a
fresh pass in this visit. Fresh Rust checks are unavailable here because
`cargo` is absent from the workspace tool's box; strict OpenSpec and diff
checks pass. External exact coverage, macOS and remote CI remain pending.
