...
- [ ] 1.1 Delete `confined_command` (`crates/brokkr-runtime/src/engine.rs`)
...
- [ ] 1.2 Delete `crates/brokkr-runtime/tests/confine_test.rs` and the
...
- [ ] 1.3 Add the parser refusal of `driver.confine` at any site, in a
...
- [ ] 1.4 Tests in `crates/brokkr-runtime/src/bundle/tests.rs` (and
      ...
      refused by name.
- [ ] 1.5 A source-scan test that `docker run`, `confined_command` and
...
- [ ] 2.2 Add the `Option<Boundary>` serde helper that reads and writes
...
- [ ] 2.5 Teach the map loader `forge.realms/v4`: `Realm.boundary:
      ...
      forge.realms/v4.
- [ ] 2.6 Loader tests: a v4 map declaring five realms with the five
...
- [ ] 2.7 Add the `realms.v4` entry to
...
- [ ] 3.1 Give `Bundle` a `boundary` field set by
...
- [ ] 3.2 Wire the verbs in `crates/brokkr-cli/src/lib.rs`: `run` and
...
- [ ] 3.3 Change `Cmd::Rerun` to discover the workspace map, compile with
      ...
      realm-boundary / A run's boundary is the realm's, resolved at compile.
- [ ] 3.4 Tests: `run` under a v4 map declaring `harness` compiles a
      ...
      resolved at compile.
- [ ] 3.5 Refuse a `boundary` key in the bundle parser at every site — a
...
- [ ] 3.6 Tests in `bundle/tests.rs` and `bundle/agent_tests.rs`: a seat,
...
- [ ] 3.7 Confirm `brokkr compile` prints the manifest and nothing else
...
- [ ] 4.1 Write `contracts/run-manifest.v9.schema.json` as v8 plus
...
- [ ] 4.2 Write `boundary` in `manifest_for`
      ...
      manifest pins the boundary per site as run-manifest/v9.
- [ ] 4.3 Tests: two boxed sites give two `boundary` keys equal to the
      ...
      manifest pins the boundary per site as run-manifest/v9.
- [ ] 4.4 Add the `run-manifest.v9` entry to `frozen_contracts.rs`: the
...
- [ ] 4.5 Carry `boundary` through `bundle_manifest_from_run`
...
- [ ] 4.6 Make `build_run_manifest_v2` refuse a bundle manifest carrying
...
- [ ] 4.7 Tests: a run started under `namespace` handed a bundle compiled
...
- [ ] 5.1 Add `offered(path) -> BTreeMap<Boundary, Offer>` with
...
- [ ] 5.2 Rewrite `refuse_unboxable` (`crates/brokkr-cli/src/lib.rs`) over
      ...
      refuses at start.
- [ ] 5.3 Add the unbuilt refusal after the tool check: `seatbelt` and
      ...
      slice lands.
- [ ] 5.4 Call `refuse_unboxable` from `run`, `resume` and `rerun` before
...
- [ ] 5.5 Add the engine's own entry fences: `Engine::start_in_world`,
...
- [ ] 5.6 Availability tests, one per boundary on an empty search path:
...
- [ ] 5.7 Unbuilt tests: `seatbelt` with a planted `sandbox-exec` still
...
- [ ] 5.8 Fence tests in the engine's tests: a `harness` bundle started
      ...
      under.
- [ ] 5.9 Verb tests: `brokkr run`, `brokkr resume` and `brokkr rerun`
      ...
      refuses at start.
- [ ] 5.10 Add doctor's one `boundaries` line from the same `offered` map
      ...
      machine offers.
- [ ] 5.11 Make `doctor --bundle` compile in the discovered realm and
      ...
      offers.
- [ ] 5.12 Doctor tests in `crates/brokkr-cli/tests/init_doctor.rs`: a
      ...
      boundary-availability / doctor names the boundaries this machine
      offers.
- [ ] 5.13 Rewrite `brokkr init`'s bubblewrap warning
...
- [ ] 8.10 Deliver the judge's result under `harness`: the engine expands
      ...
      gate-boundary-policy / A judge under harness still delivers its
      result file.
- [ ] 8.11 Door tests: a codex gate whose result path is `P` composes
...
- [ ] 9.1 Build `boundary_entries` beside `select_candidates` in
...
- [ ] 9.2 Publish `contracts/effect-boundary.v1.schema.json`, title
...
- [ ] 9.3 Effect tests: a gate-class boxed single seat under `harness`
      ...
      provenance.
- [ ] 9.4 Write `contracts/seat-record.v4.schema.json` as v3 plus the
      ...
      seat-record/v4.
- [ ] 9.5 Add `SeatRecordVersion::V4` dispatched by the 0.9 line (D9): an
...
- [ ] 10.1 Add `ModelAtBoundary { model: Cell, boundary: Cell }` to
      ...
      every model cell.
- [ ] 10.2 Derive the cells beside the provenance scan: a participant's
...
- [ ] 10.5 View tests: a boxed seat under `harness` reading `harness`
      ...
      boundary-readouts / The view derives the boundary beside every model
      cell.
- [ ] 10.6 Print the run-level fact from that one derivation on every
      ...
      boundary-readouts / A run whose gate stood under harness or open is
      rendered unboxed.
- [ ] 10.7 Add the one pair helper — a text face that prints the model
...
- [ ] 10.12 Test `brokkr export` as the record itself: the exported
      ...
      model names its boundary.
- [ ] 10.13 Teach `scripts/delivered-by-brokkr.sh` to read the anchored
...
- [ ] 11.1 Re-pin every moved witness digest in
...
- [ ] 11.2 Name decision 0046 in each pin file's doc comment as the reason
      ...
      boundary-manifest-pin / Every pinned digest that moves is re-pinned
      with 0046 as the reason.
- [ ] 11.3 Check the fixed points: every pinned bundle that declares no
...
- [ ] 12.1 `docs/guides/provider-adapters.md`: document `hands.harness`
...
- [ ] 12.2 `docs/guides/recipe-authoring.md`: the `hands` row stops saying
      ...
      boundary-guides / The guides document the boundary and never lose a
      section.
- [ ] 12.3 `docs/guides/quickstart.md`: rewrite the platform paragraph to
...
- [ ] 12.6 `docs/guides/repository-layout.md`: name `boundary` beside
      ...
      boundary-guides / The guides document the boundary and never lose a
      section.
- [ ] 12.7 `docs/guides/driver-authoring.md` and `ARCHITECTURE.md`: stop
...
- [ ] 13.2 `cargo clippy --workspace --all-targets --all-features -- -D
      warnings` clean — every requirement of this change.
- [ ] 13.3 `cargo test --workspace --no-fail-fast` green as one
...
- [ ] 13.4 `cargo package --workspace --allow-dirty` verifies — every
      requirement of this change.
- [ ] 13.5 `brokkr compile --bundle bundles/self` and `--bundle
      bundles/verify` both compile — realm-boundary / brokkr compile prints
      the boundary under each hands site.
- [ ] 13.6 `scripts/coverage-exact.sh` at literal 100% of lines and
...
- [ ] 13.8 Commit the work in the repository's message style, and never
