...
| B36 | the engine-side route cases belong to `resume_tests.rs` "in the pattern of the private-context unit case", built through that suite's `bundle` helper, with a dsh site carrying one `--patch`, the logging driver, the planted file, read off `Start.input` "at both production `start_context` call sites… and for the positive case on a cold start, an offered start and a start under an `unmeasured` assessment alike" | 5213–5229 | `resume_tests.rs` 1609–1930: `overlay_digest`, `patched`, `route_start`, `route_binding`. The positive cases opened one by one: `:1644` `::a_valid_route_overlay_binds_at_the_single_site` — `single(patched(…))`, inline, so cold under an `unmeasured` assessment; `:1688` `::…_at_the_panel_member` — `panel(…)`, also cold/`unmeasured`; `:1866` `::a_valid_route_overlay_binds_on_an_offered_start_too` — the retry that receives the offer, but built with **`seat(single(argv, vec![candidate]), …)`** at `:1891` | **partially discharged.** The clause asks for the positive "at both production `start_context` call sites, a single site and a panel member, **and** for the positive case on a cold start, an offered start and a start under an `unmeasured` assessment alike" — a 2 × 3 matrix. Opened: single/cold-`unmeasured`, panel/cold-`unmeasured`, single/offered. **Missing: the offered start at the panel member.** The first cut read the three cases as covering both axes because each axis appears somewhere; they do not meet at that cell. Unit 2a |
...
| B59 | "independently exercise matching observations, absent/malformed/unreadable version output, version-command failure and version drift, producer error and canonical-composite mismatch. A matching version invokes the sole producer once; earlier failures invoke it zero times." | 5302–5306 | `::a_dsh_identity_mismatch_declines_the_offer_and_keeps_the_cold_route`, opened at `adapters/tests.rs:10406–10471`. Present: producer error (`:10408`), canonical-composite mismatch (`:10419`), **version-command failure** (`exit 3`, `:10428–10441`), **one** successful probe whose banner reads `no-version-here` (`:10441–10451`), and version drift with `calls == 0`; the matching case is `::a_qualified_dsh_launch_…` | **partially discharged.** The clause names five enabled-path classes to exercise "**independently**": matching, **absent**, **malformed**, **unreadable** version output, and version-command failure. Four have vectors; the middle three share **one**. The `no-version-here` banner is a successful exit whose stdout is non-version text — naturally read as *malformed*. There is no **absent** vector (a probe succeeding with empty stdout) and no **unreadable** vector distinct from it (non-UTF-8 or an unreadable stream). Both assert `observed == None` in production (`adapters.rs` parses the banner), so this is **missing evidence, not missing behaviour** — but "independently" is the clause's own word. Unit 3 |
