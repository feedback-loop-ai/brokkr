...
| A37 | **(d)** bind the seat's single `--patch` "at every model-site start, cold, offered or `unmeasured`, independent of the resume gate, **at both `start_context` call sites**", withholding it for an absolute path, `..`, symlink escape or a non-member | 4905–4917 | Positive: `resume_tests.rs::a_valid_route_overlay_binds_at_the_single_site` (`:1687`), `::…_at_the_panel_member` (`:1731`), `::a_valid_route_overlay_binds_on_an_offered_start_too` (`:2246`, single site), `::a_valid_route_overlay_binds_on_an_offered_panel_member_start_too` (`:2310`). Withholding: `::a_non_binding_route_overlay_withholds_the_member_at_both_call_sites` (`:1778`, panel member), `::…_at_the_single_site` (`:1862`), `::every_remaining_non_binding_shape_is_withheld_at_the_single_site` (`:1919`), `::an_escaping_symlink_member_is_withheld_at_the_single_site` and `…_at_the_panel_member` (`:2040`, `:2087`) | discharged (corrected by unit 2d) — the positive binds at both call sites on cold `unmeasured` starts and on an offered start at both: the single site's is a retry, the panel member's a re-entry offered `alpha-1`, each start carrying the argv value and the manifest digest. Before 2d the offered positive rode the single site alone. Every withholding shape the clause names — absolute, `..`, compiled-member symlink escape, non-member — is withheld at both call sites (see B38). Units 2a, 2b, 2d |
...
| B38 | "(ii) A same-shaped nonmember outside the layer, a working-directory shadow…, an ancestor-layer file, a `..` component and an in-layer symlink whose target resolves outside the layer directory while remaining inside the working directory (**present at compilation and therefore a `files` member**), and the absolute path produced by `./` expansion each receive no binding… **at either call site**." | 5231–5237 | **Panel member:** `::a_non_binding_route_overlay_withholds_the_member_at_both_call_sites` (`resume_tests.rs:1778`) — six shapes (`nonmember`, `shadow`, `ancestor`, `traversal`, `absolute`, nonmember `symlink`); `::an_escaping_symlink_member_is_withheld_at_the_panel_member` (`:2087`, unit 2b). **Single site:** `::a_non_binding_route_overlay_withholds_the_member_at_the_single_site` (`:1862`, nonmember, unit 2a); `::every_remaining_non_binding_shape_is_withheld_at_the_single_site` (`:1919`, shadow, ancestor, traversal, absolute, unit 2d); `::an_escaping_symlink_member_is_withheld_at_the_single_site` (`:2040`, unit 2b). The `./` half is `bundle.rs:3689`, where a `./`-spelled command part expands to `dir.join(rel)` — an absolute path, which is the `absolute` shape | discharged (corrected by units 2a, 2b and 2d) — every named shape receives no binding at each call site. Each case reads the real `Start.input` and finds the context present and carrying no `route_overlay`. The escaping symlink is a compiled `files` member at both sites (2b). At the single site each of 2d's vectors names bytes a lookup could find: the shadow and the ancestor file are spelled `route.yml` beside a member of that name, and the ancestor records its file in its own `files`. The `..` value and the absolute expansion both resolve to the member. A compiling mutation of each rule parts that shape alone. **Residual, not a clause gap:** at the panel member the `traversal` value (`../escape.yml`) names no file and the ancestor's `files` is empty. Those two withhold there without isolating their rule: the `..` and ancestor-fallback mutations leave `:1778` green. The clause asks only that they receive no binding, and they do |
...
2d. **Close the route-binding matrix at the single site.** Added from the
   ...
   **Landed 2026-09-23 at `97624edd`; no shape binds — no Pass-B finding.**
   ...
   mutations; B38 records this as a residual. `cargo fmt --all -- --check`,
   `cargo clippy --workspace --all-targets --all-features --locked -- -D
   warnings` and `cargo test -p brokkr-runtime --all-features --locked`
   (463 lib plus 94 integration passed, 0 failed) are green. Tests only; no
   production line moved; the DSH route stays disabled; no checkbox moved.
