...
|---|---|---|---|---|
| A1 | "Execute local groups 8.8.9–8.8.15 above" | 4736 | Rows S1–S13 below | partially discharged — *entry 22, 2026-09-23:* S1–S11 and S13 now read discharged; **S12 (8.8.15.1) remains open**, because this seat's grant refused its strict validation (§6, entry 22). *Before entry 22:* S11 (8.8.14.2) and S12 (8.8.15.1) remain open. This clause names groups 9–15 only; it does not narrow 8.8's acceptance to them, and §1's fourteen numbered tasks stay in force (1209–1212) |
...
| S12 | 8.8.15.1 **`[ ]`** — record the delivery, "Tick each finished scoped task beside its evidence", verify `git diff --check`, strict active-change validation, the requirement/checkbox inventory, "then commit the exact staged repair and ledger"; "Do not report delivery before that commit or represent pending external gates as passed" | 327–345 | The three Pass D delivery records exist (`tasks.md` 535, 703, 879) and the clause's own gate — "After the scoped implementation/proofs **and local gates above pass**" — is S11, which has not | **discharged — regraded by entry 22's closing visit, 2026-09-23; 8.8.15.1 ticked.** The missing verification now exists: the host ran `openspec validate --all --strict` (Totals 17 passed, 0 failed, rc 0) and `openspec validate 2026-09-09-226-session-resumption --strict` (valid, rc 0) with openspec 1.12.0 on head `0046ef43`, entry 22's first commit (`.forge/tasks/host-openspec-0046ef43.txt`, opened). The closing commit moves only `tasks.md`'s ticks and prose and this ledger; `proposal.md`, `design.md` and the spec deltas are byte-identical to `0046ef43`, so the validated bytes are unchanged, but the run is on `0046ef43`, not on the closing commit, and was not rerun. *Grade before the closing visit, kept:* **partially discharged — entry 22, 2026-09-23; 8.8.15.1 NOT ticked.** S11 now passes, the delivery is recorded (`tasks.md`'s entry-22 section), 8.8.14.2 is ticked beside its evidence, `git diff --check` passes, the checkbox and requirement inventory is taken, frozen surfaces are unchanged, 0056 is `proposed` and the DSH route is `unmeasured`. One of the clause's own verifications is missing: **strict active-change validation on the candidate that carries these edits.** This seat's grant refused `openspec validate --all --strict` and `openspec validate 2026-09-09-226-session-resumption --strict`. The host's 17/17 is on `43d8b8b8`, before three documents of this change moved. An unavailable tool is not a pass, so the clause stays open until that run exists (§6, entry 22). *Before entry 22:* **not started** — blocked on S11 by its own first words. Note what it does and does not require of external evidence: it asks that "external pending evidence" be **recorded**, not completed, so it can close over a pending N13. See unit 22 |
...
| B2 | "execute only 8.8.9.1–8.8.15.1: R1–R3 plus the bounded watcher-rule controls, both terminal endings, completed production observations, independent compiling removals and positive/cold preservation" | 5089–5093 | Rows S1–S13 | **discharged — regraded by entry 22's closing visit, 2026-09-23**, with S12, on the host's strict validation of `0046ef43` (`.forge/tasks/host-openspec-0046ef43.txt`, opened: 17 passed, 0 failed; the single change valid; both rc 0). *Grade before the closing visit, kept:* partially discharged — *entry 22, 2026-09-23:* every removal-bearing S row is now discharged under entry 17's ruling, and S11 on entry 14's host run. **S12 alone keeps this row open**, until strict validation runs on the candidate carrying entry 22's edits (§6, entry 22). *Before entry 22:* S11 and S12 are open, and every removal-bearing row among S1–S9e carries an evidence-verification gap (F5). Note the scope word: 8.10 says "execute **only** 8.8.9.1–8.8.15.1", so §1's numbered tasks are not 8.10's work — they bear on 8.8's tick, not this one's |
...
| B10 | "Clauses 5–6 own fresh gates, coverage and the unsigned evidence commit." | 5105–5106 | Rows S11, S12 | **discharged — regraded by entry 22's closing visit, 2026-09-23.** S12's strict validation now exists on the host's run of `0046ef43` (`.forge/tasks/host-openspec-0046ef43.txt`, opened: 17 passed, 0 failed; the single change valid; both rc 0), so 8.8.15.1 ticks. The unsigned evidence commit is the closing visit's own, unsigned under CONTRIBUTING.md §5 ("Seat commits are unsigned"); its hash is handed off in the run's result. *Grade before the closing visit, kept:* partially discharged — *entry 22, 2026-09-23:* the fresh gates are discharged (S11, entry 14's host run on `43d8b8b8`). Exact coverage passed on the same host and revision at 32,458/32,458 lines, 5,508/5,508 branches and 3,161/3,161 functions (`.forge/tasks/entry14-43d8b8b8/coverage.out`, opened). **The unsigned evidence commit is open**, because S12 is: its strict validation was refused to this seat. Entry 22's own commit is unsigned, but 8.8.15.1 is not ticked in it. *Before entry 22:* the gates and the commit are open |
...
| B20 | "pair its selector mutation with the existing protocol adapter test `a_refused_resume_is_a_cold_spawn_with_the_refusal_journaled`, verifying two children, no selector and the retained sandbox in the replacement argv, and one cold launch row" | 5131–5135 | That test exists, asserts two children, no selector, the retained sandbox and one cold launch row, and ran green | **discharged — regraded by entry 22, 2026-09-23, on entry 19's replay.** Opened in `removal-controls-2026-09-23.md`, E19-5. With `codex_selector_conflict` made never to match (`adapters.rs:2502`), `a_codex_seat_argv_that_selects_a_session_is_refused_on_the_cold_path_too` parts at `adapters/tests.rs:1845` (`left: None`, `right: Some("resume")`). The paired `a_refused_resume_is_a_cold_spawn_with_the_refusal_journaled` passes beside that mutation. A temporary diagnostic, printed after its last assertion and since removed, shows two children, no `resume` selector in the replacement, `--sandbox read-only` retained, and one launch row, `launch: cold`. *Pre-regrade grade, kept:* **partially discharged — evidence-verification gap.** The test is opened. "**pair its selector mutation with**" that test is a narrative record (F5). It closes on a unit-17 ruling that names it, or by unit 19 |
...
| B39 | "(iii) A member whose bytes changed after compilation cannot authorize its new bytes: the carried digest is the manifest's recorded value, not a hash of the resolved file, **at either call site**." | 5237–5240 | `::a_changed_route_overlay_member_carries_the_manifest_digest` — asserts the manifest digest and `assert_ne!` against a hash of the changed bytes | **discharged — regraded by entry 22, 2026-09-23.** Opened: `::a_changed_route_overlay_member_carries_the_manifest_digest_at_the_panel_member` (`resume_tests.rs:2216–2258`) drives a `panel(vec![member(…)])` seat whose member's file changed after compilation, and asserts the manifest's digest and `assert_ne!` against a hash of the file read back off disk. With the single-site case at `:2165`, both call sites are proved. *Pre-regrade grade, kept:* **partially discharged** — its seat is `single(…)`, so the case is proved at the **single site** only; the panel member's changed-bytes variant is not driven. The mirror of B38's gap. See unit 2a (now 2), whose record at `17b5b4d2` adds `a_changed_route_overlay_member_carries_the_manifest_digest_at_the_panel_member`. The row keeps this grade until unit 22 regrades it on opened evidence |
...
| B99 | "keep 8.8's committed-bytes test pinning the repository-owned adaptation's exact six-file set against its provenance block" | 5437–5439 | Row A67 | **discharged — regraded by entry 22, 2026-09-23**, with A67 (`composite/tests.rs:949–959`, opened). *Pre-regrade grade, kept:* partially discharged — see A67 and F1. Unit 1 has landed. The row keeps this grade until unit 22 regrades it on opened evidence |
...
| 14 | 4 | run green by the host on `43d8b8b8` (`.forge/tasks/entry14-43d8b8b8/`), recorded by entry 22 |
| 15 | 5 | coverage fix landed `6d316980`; the host's exact coverage is 100% on `43d8b8b8`; PR #327's CI on `43d8b8b8` is as reported to entry 22, not opened by it; externally owned |
| 16 | — (new: N11 on a capable host) | **pending**, externally owned: no host with `node` on `/bin:/usr/bin` has run it |
| 17 | 9 | ruled 2026-09-23 (`4d2e9e72`): yes for 27 rows, no for B16 and B20 |
| 18, 20 | — (new: replays after a negative ruling) | no work under 17's ruling |
| 19 | — (new: replays after a negative ruling) | landed `7f2aaeda`, recorded `d6fc4ed3` |
| 21 | — (new: (a) THE PROOFS' unrecorded controls; (b) the remaining replays) | (a) landed: performed at `90b548e3`, closed under the operator's 2026-09-23 ruling on F1–F4; (b) no work under 17's ruling |
| 22 | 6 | regrade and record landed 2026-09-23, with 8.8.14.2 ticked; **8.8.15.1 and 8.10 not ticked**, because strict validation was refused to the seat |
...
22. *(recorded as 6)* **Regrade, record, tick and commit the D7/Pass-C/Pass-D
   ...
   *(Third return, finding 3: the previous cut let 8.10 tick after a negative
   ...
   - 2026-09-23, **entry 22 regraded and recorded; 8.8.14.2 ticked;
     8.8.15.1 and 8.10 NOT ticked** (run
     `issue-226-acceptance-ledger-entr-b22f02b9`). The commit is the one
     carrying this line, unsigned, whose parent is `d6fc4ed3`. Its hash is
     handed off in the run's result rather than written here, as 8.8.15.1
     asks ("hand off its hash without another tracked write"). Every
     condition of this entry's 8.10 list holds on opened evidence:
     - entry 14 is green (`gates.log`, every command `rc=0`, openspec
       17/17);
     - entry 11 has landed (`:3202`, opened);
     - 21(a) has landed, both unwrapped Codex controls included;
     - entry 17's no for B16 and B20 is answered by entry 19's five
       controls, each at its named assertion. The ruling's yes names every
       other 8.10 removal predicate.

     All thirteen named rows and every other row a landed entry closes are
     regraded to discharged on opened tests or records: A42, A67, B27,
     B39, B47, B49, B50, B60, B71, B99, A13 and B13, and A64's 8.8 half.
     So are A53, B30, B42 and B55 on entry 11, B16 and B20 on entry 19,
     and A61, B5, B77, S1 and S3–S10 on the ruling. S11 is discharged on
     entry 14's host run, and 8.8.14.2 is ticked beside it.

     **What stopped the other two ticks.** This seat's grant refused
     `openspec validate --all --strict` and the single-change form. 8.8.15.1
     requires strict validation of the candidate it commits before its own
     tick. The host's 17/17 is on `43d8b8b8`, before this change's
     documents moved, and an unavailable tool is not a pass. So S12 stays
     open. B2 and B10 ride it, and they are the only 8.10 rows not reading
     discharged, withdrawn, or not this slice's (B26, B101). **What
     remains is one step.** Run `openspec validate --all --strict` on this
     commit. If it passes, tick 8.8.15.1 and 8.10 beside that result and
     regrade S12, B2 and B10. Nothing else is owed. Gates run here:
     `git diff --check`, `cargo fmt --all -- --check`, the checkbox and
     requirement inventory, the frozen-path diff against `origin/main`
     (empty), 0056 `proposed`, and `adapters/dsh.json` `headless-work`
     `unmeasured`. Entry 16 is recorded **pending**, never passed. PR #327's
     CI is recorded as reported: this seat's grant also refused
     `gh pr checks`.
   - 2026-09-23, **entry 22 closed; 8.8.15.1 and 8.10 ticked** (run
     `issue-226-acceptance-ledger-entr-aaa075ff`). The host ran the one
     remaining step on `0046ef43`, the first visit's commit:
     `.forge/tasks/host-openspec-0046ef43.txt` records `openspec validate
     --all --strict` → 17 passed, 0 failed, rc 0, and `openspec validate
     2026-09-09-226-session-resumption --strict` → valid, rc 0, openspec
     1.12.0. 8.8.15.1 is ticked beside that record, then 8.10. S12, B2 and
     B10 are regraded discharged, each keeping its old grade. The closing
     commit is the one carrying this line, unsigned, whose parent is
     `0046ef43`; its hash is handed off in the run's result. It moves only
     `tasks.md`'s ticks and prose and this ledger. The validated deltas are
     byte-identical, but the host's run is on `0046ef43`, and this visit did
     not rerun openspec. Commits: `0046ef43` (8.8.14.2) and this one
     (8.8.15.1, 8.10). Nothing else is ticked: 8.8, 9.6, 11.x, §1's
     numbered rows and groups 14 and 15 stay open, and entry 16 stays
     pending. Gates: `git diff --check` and `cargo fmt --all -- --check`
     pass; the frozen-path diff against `origin/main` is empty; 0056 is
     `proposed`; `headless-work` is `unmeasured`.
...
- **8.10 — Yes, conditionally.** It ticks only after entries 11, 14 and
  ...
  *(2026-09-23, entry 22: 14 is green, 17 is ruled, and 19 answers its no,
  so every predicate above holds and every stale row is regraded. 8.10 is
  still unticked, for one reason only: 8.8.15.1's strict validation was
  refused to the seat, and B2 and B10 ride that clause. One granted
  `openspec validate --all --strict` on entry 22's commit ticks both.)*
