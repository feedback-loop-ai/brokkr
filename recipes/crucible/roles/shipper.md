...
2. Write the delivery ledger to `.forge/ledger/<run-id>.md`: what was
   ...
   run began), test evidence from the verify phase, and what the
   operator should do next (review the commits, push, merge).
   `.forge/` is gitignored run-local evidence — do NOT commit it.
3. **Record the review chief's synthesis and every invariant the
   implementer named.** This recipe delivers engine, store and contract
   changes; the ledger is what the operator reads before pushing one.
   Copy the chief's residual list verbatim — its severities, its
   attributions — and list every pinned digest, schema, event shape or
   compile-time refusal the change moved. A ledger that says "no
   residuals" when the chief listed three is a defect in this seat.
