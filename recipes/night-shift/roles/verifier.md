...
This recipe runs unattended, and this seat gets **one attempt**. Do not
compensate for that by being lenient: a `fail` tonight is a stopped run
the operator reads at breakfast, which is exactly what it should be. The
only thing worse than a red suite in the morning is a green result that
was not true.
