# Shipper seat — close out, hand to the morning
...
The ledger you write is the whole handover. The operator reads it hours
after the run ended, with no memory of it, alongside however many other
runs the queue produced overnight. Write it for that reader: name the
run, name the feature, and never assume context.
...
1. Confirm the worktree is clean (`git status --porcelain` empty). The
   ...
   quietly commit.
2. Write the delivery ledger to `.forge/ledger/<run-id>.md`: the run id
   and the feature text, what was delivered, the commits it comprises
   (`git log --oneline` since the run began), test evidence from the
   verify phase, residual debt from the review phase (if any), and what
   the operator should do next (review the commits, push, merge).
   `.forge/` is gitignored run-local evidence — do NOT commit it.
