...
| [0042](0042-the-council-is-brokkrs-the-dialect-is-the-realms.md) | The council is Brokkr's, the dialect is the realm's: spec-driven delivery speaks OpenSpec or spec-kit, and the tool validates | The design council stays; where its artifacts live is a realm fact. `realms.json` declares a dialect from a data library (`openspec` 1.12.0, `speckit` 0.8.7, both measured), a design phase compiles only against one, the dialect's own validator runs as a boxed exec gate and its close-out is the smith's last task, the change id is a typed identifier input every later seat reads from the journal, and this repository speaks OpenSpec with its hybrid layout retired to the evidence tree. | proposed |
