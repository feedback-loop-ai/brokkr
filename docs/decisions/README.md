...
| [0035](0035-effort-is-part-of-the-hire.md) | Effort is part of the hire | A model name alone does not identify the worker: seat record v2 adds the served `effort` and `reasoning_output_tokens`, every model pin carries an effort pin, and explicit pinning over implicit defaults is stated as the ledger's general rule. | proposed |
