...
| [0042](0042-the-council-is-brokkrs-the-dialect-is-the-realms.md) | The council is Brokkr's, the dialect is the realm's: spec-driven delivery speaks OpenSpec or spec-kit, and the tool validates | The design council stays; where its artifacts live is a realm fact. The SDD machine is a table with three artifact phases — specify (the what), design (the high-level how), tasks (the low-level how) — each bounded and returnable; a dialect maps a framework's own artifact graph onto them, its sub-machine as steps composed into each phase's sequence, checked at compile time against the framework's order. `realms.json` declares the dialect from a data library (`openspec` 1.12.0, `speckit` 0.8.7, both measured), each phase ends in the tool's own validator as a boxed exec gate with the framework's state as evidence, the change id is a typed identifier input every later seat reads from the journal, the smith folds the change, and this repository speaks OpenSpec with its hybrid layout retired to the evidence tree. | proposed |
