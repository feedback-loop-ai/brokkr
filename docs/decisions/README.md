...
| [0048](0048-the-script-directory-is-the-spawn-pin.md) | The script directory is the spawn pin | Preserve exact filename components and check the script directory against its compiled file map at unboxed spawn, allowing an in-place realm's source, results and journal to change. | proposed |
| [0049](0049-the-platform-classes.md) | The platform classes: what each operating system is promised, and what is said where a promise cannot hold | Linux is first-class and defines every guarantee, macOS is supported, Windows is best-effort: tested on every pull request and never distorted for. Where an operating-system behaviour defeats a guarantee, the guarantee is named as not holding there rather than pursued, and the layer that dispatches the run carries what the platform cannot. The pinned-script rule is integrity over the bundle's bytes and an execution guarantee only under a boundary that supplies the filesystem and the PATH. | accepted |
| [0050](0050-the-machine-proves-what-it-promises.md) | The machine proves what it promises: presence, order and totality at load, every ending named, and the inner machines declared | The three loader refusals decision 0004 deferred — presence, order, liveness — a compile-time sweep that refuses an unruled valuation and names every closed ending with a parking rule, stated properties over every shipped table, and the fold's arms and the sequence's endings declared and total; the frozen table and corpus untouched. | proposed |
| [0051](0051-landing-a-branch-written-by-hand-is-judged-and-shipped-by-a-run.md) | Landing: a branch written by hand is judged and shipped by a run, and the label goes back to being the exception | `recipes/landing` is `fast` entered at a classify gate: prose goes to review, code to verify, findings return to the smith, and ship anchors the head so the contribution gate vouches for hand-authored work without the by-hand label; the label is the residue and stays counted. | proposed |
