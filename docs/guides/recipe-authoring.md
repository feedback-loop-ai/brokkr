...
| `hands` | Decision 0040: `"workspace"` or `{"kind":"workspace","network":bool,"binds":[{path,mode,mask}]}`. The seat's commands run inside an empty-root box holding only the worktree; a tool allow-list is not consulted, and a boxed `exec` command may hold a gate. Refused beside secret bindings and beside `agent:` (the agent declares its own). |
