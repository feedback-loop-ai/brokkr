...
seat live across the drift between its 0.148.0 measurement and the earlier
0.153.4 applicability, now reconciled to the exercised 0.154.0 by the
2026-09-16 live proof, with every observed identity, boundary, hands and
...
|---|---|---|---|
| `codex` | `work-site` | `supported` | **Preserved, not newly enabled.** Main already rejoins this harness work seat under accepted decision 0030, and the operator's 2026-09-15 ruling keeps it live across the drift between 0030's **0.148.0** measurement and the earlier **0.153.4** applicability. The 2026-09-16 live proof on the exercised **0.154.0** reconciles both declared identity fields to **0.154.0** and records the exact invocation, restriction, same-root, current-only accounting and pre-work rejection axes. The declaration scopes the measurement to `boundaries: ["harness", "not applicable"]` with `hands: "none"` — the engine-composed harness work seat main runs (`--sandbox workspace-write`, no boxed-hands marker) beside the author-written inline coordinate main also rejoins (`boundary: not applicable`, the argv's own `--sandbox` class). Because `codex exec resume` offers no sandbox flag (`-s` is rejected), the class can only travel as `-c sandbox_mode=<class>`, which the adapter emits. Observed identity must still equal the declared applicability **0.154.0** and, where the offered root recorded one, the same opening version: a **0.148.0** or **0.153.4** observation or recorded origin still refuses, and every boundary, hands, ownership and accounting mismatch still refuses. A boxed Codex work seat keeps its MCP `-c` fragment, which main's resume allow-list already refuses; this declaration declines that coordinate as `restrictions-unavailable`. The 2026-09-17 interface record measures effort configuration (`model_reasoning_effort` accepted under `--strict-config`, with a deliberate misspelling refused as the control), the resume option surface and the stdin `-` positional on **0.154.0**. Task 11.1 stays owed even so: membership in the surface the subcommand parses is not qualification to travel on a resume, and `--ephemeral` and `--output-schema` are admitted by the allow-list while no supplied observation establishes the persistence, restriction, root or accounting behaviour of either — preservation plus five measured live axes plus a measured interface is still not full installed-version qualification. |
