...
| `dsh` | `headless-work` | `unmeasured` | The exact route is selected: official core **0.1.5-rc.1** at `183f08e9c6dde7e36cd2318eaee70b0da08fb35e` with `dsh-plugin-cli-session` **0.2.0** at `0f487e74c81ed102c6899440d9f5d65e8e9eabda`, composed only in worktree- or task-owned storage through DSH's documented extension and `agents.resume` APIs. Its resolved-composite compatibility, provider-confirmed exact-root rejoin, current headless model/effort and restriction precedence, and noncumulative current-sequence accounting are unmeasured, so the shape stays disabled. The plugin's request-derived `session_id` echo and post-`firstSeq` usage selection are interface evidence only. The installed 0.1.2-rc.1 one-shot runner mints a fresh root and cannot forward the launcher's TUI example; that is bounded history for that entry, never a global DSH or extension limitation. The separate hands/tools plugin deferral is unrelated. |
