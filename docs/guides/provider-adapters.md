...
| `dsh` | `headless-work` | `unmeasured` | The exact route is selected: official core **0.1.0-rc.6** (`@deepseek-ai/dsh@0.1.0-rc.6`, the generation the plugin's own development matrix pins) with `dsh-plugin-cli-session` **0.2.0** at `0f487e74c81ed102c6899440d9f5d65e8e9eabda`, composed only in worktree- or task-owned storage through DSH's documented extension and `agents.resume` APIs. An isolated live cold+warm probe (2026-09-10) emitted successful result envelopes, continued one root, recalled the nonce and reported per-message usage with the global pin unchanged. Consumer-side exact-root confirmation, current headless model/effort and restriction precedence, and a multi-message/retry current-sequence accounting boundary are unmeasured, and the Rust route is unimplemented, so the shape stays disabled. The plugin's request-derived `session_id` echo and post-`firstSeq` last-wins usage are interface evidence only. Core 0.1.5-rc.1 broke the plugin's post-turn fold by removing `agent.session.events`; that is a measured pair incompatibility, never a global DSH or extension limitation, and the installed 0.1.2-rc.1 one-shot runner remains bounded history for that entry. The separate hands/tools plugin deferral is unrelated. |
