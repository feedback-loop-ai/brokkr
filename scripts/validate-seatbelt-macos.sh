...
exec 3>&1 4>&2
exec > >(tee "$EVIDENCE_ROOT/runner.log") 2>&1
TEE_PID=$!
cd "$REPO"
{
  date -u
...
rustc +stable --version --verbose >> "$EVIDENCE_ROOT/host.txt"
...
rustc +stable --edition 2021 -D warnings experiments/seatbelt-lifetime/main.rs -o "$EVIDENCE_ROOT/lifetime"
