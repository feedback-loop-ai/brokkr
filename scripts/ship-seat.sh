...
while IFS= read -r line; do
    ...
    case "$trimmed" in
        ...
        '"run_id": '*) run_id="$(printf '%s' "$trimmed" | sed -n 's/^"run_id": "\([^"]*\)".*/\1/p')" ;;
        '"rule_id": '*) rule_id="$(printf '%s' "$trimmed" | sed -n 's/^"rule_id": "\([^"]*\)".*/\1/p')" ;;
    esac
done < "$prompt_file"
...
if [ "$rule_id" != "SHIP-READY" ]; then
    ...
    ledger_output="$("$brokkr" ledger --run "$run_id" --db .forge/forge.db --repo . 2>&1)"
    ledger_status=$?
    if [ "$ledger_status" -ne 0 ]; then
        printf 'ledger generation failed: %s' "$ledger_output" > "$notes_file"
    elif [ -n "$dirty" ]; then
