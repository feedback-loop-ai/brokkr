...
- [ ] **T1 — `bundle/compose.rs` with the no-`extends` path only.**
...
- [ ] **T2 — `Bundle::roots`, and `confined_command` mounts every
...
- [ ] **T3 — Name grammar and library lookup.** `extends` is validated
...
- [ ] **T4 — Chain walking, cycles, depth cap.** Each layer's own
...
- [ ] **T5 — `name` is required and must differ from every ancestor.**
...
- [ ] **T6 — Seat merge with kind-keyed markers.** `override.seats` /
...
- [ ] **T7 — The 0016 layering test.** A seat value carrying arbitrary
...
- [ ] **T8 — `policy` is per-layer.** Each layer that declares `policy`
...
- [ ] **T9 — Table merge.** Name arrays union (base order first, derived
...
- [ ] **T10 — Derived rules prepend; override-by-id is
...
- [ ] **T11 — The lint runs on the resolved table**, with zero new lint
...
- [ ] **T12 — Inherited paths resolve against their origin layer.**
...
- [ ] **T13 — `recipes/sdd-paranoid/`.** `bundle.json` with
...
- [ ] **T14 — `@compose/` manifest entries and the reserved-namespace
...
- [ ] **T15 — Base change moves the derived digest.** Mutate a base
