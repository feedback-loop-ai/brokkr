...
`crates/forge-runtime/src/agents.rs` turns *(library, adapters,
...
`contracts/README.md` made in the open. `forge-view` derives provenance
once; four surfaces render it, and `forge compare` reports a model
...
not by agent name, because that is the granularity `forge compare`
...
This is not stylistic: `parse_command` expands `{forge}` to
...
(`forge-core/src/dispatch.rs:422`) *reconstructs* the bundle manifest
...
- **Packing provenance into the existing `effect/started.driver`
  ...
  packed grammar to make a *control* decision, and `forge-bridge`
...
`forge-bridge`'s `normalize_payload` allowlist is **deliberately not
...
`forge agents list` mirrors `forge recipes list`, whose contract is
*warn on a broken entry, never abort the listing* — a single index file
makes one syntax error fatal to the whole library. `forge agents show
...
what ran and `forge agents show implementer` would be a lie for that
...
`forge agents show` never claims to show them: `results` (phase/policy
...
surface where `forge agents list` shows it every time. Collapsing
...
### Q5 — The shape of `forge agents show`
...
`forge agents list` prints one tab-separated line —
...
`forge doctor` is the real consumer of the non-`unknown` arms: it probes
...
but universally uninstalled. `forge doctor` catches that before a run
...
- **AC-3 — An optional capability gap WARNS and lands in the
  ...
  `forge inspect` as a run-level notice too (AC-17).
...
- **AC-8 — Provenance in every readout.** `forge inspect --json`,
  `forge tui`, the `forge ui` console and `forge compare` all show
  agent/model/provider from the single `forge-view` derivation; no
...
- **AC-10 — CLI.** `forge agents list` lists every agent and warns
  without aborting on a broken file; `forge agents show <name>` prints
  the definition plus its per-entry resolution; an unknown name errors
  naming the known set; `forge doctor` reports providers, their
...
- **AC-17 — `chosen_index > 0` is surfaced, not merely stored.** A
  ...
  run-level notice in `forge inspect`, `forge tui` and the console — a
  notice that only lands in JSON nobody reads is the ruling's "never
  nothing" defeated.
- **AC-18 — `forge compare` reports resolution divergence.** A
...
- **AC-19 — The Looper lineage refuses rather than truncates.**
  ...
  exactly as today. `forge-bridge`'s allowlist is asserted to still drop
...
4. **Compilation does not fail for a mapped-but-uninstalled chain**,
   because compile supplies no availability facts. `forge doctor` before,
